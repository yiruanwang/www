...
export default {
 data(){
  ...
  mounted() {
    console.log('IndexBigImg mounted');
  ...
  methods: {
    ...
    textFadeInAndOut(
      ...
      descFadeInTime = this.descFadeInTime,
      descFadeOutTime = this.descFadeOutTime,
      descNextTime = this.descNextTime,
    ) {
      ...
      console.log('下面代码只执行一次');
      //   if (!this.isFirst) return // lock 初次执行
      // 没有节点创建节点
      if(!descElement){
        let hero = document.getElementsByClassName("hero")[0]
      ...
      if (descElement) {
        ...
        line && descElement.appendChild(line); // 创建 desc 所在的新元素
